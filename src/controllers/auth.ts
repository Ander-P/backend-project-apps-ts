...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    const userQuery = await sdkAdmin
      .firestore()
      .collection("Users")
      .where("email", "==", email)
      .limit(1)
    ...
    res
      .status(400)
      .json({ msg: `${errorInfo || "An unexpected error occurred"}` });
  }
};
...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({ msg: "Usuario creado correctamente", uid });
  } catch (error: any) {
    res
      .status(400)
      .json({ msg: `${getErrorCreateUser(error?.errorInfo?.code ?? error)}` });
  }
};
