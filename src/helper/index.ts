...
import { CharactersData, ComicsData } from "../types";
...
export const getComics = async (): Promise<ComicsData> => {
  try {
    const ts = new Date().getTime().toString();
    const hash = generateHash(ts);

    const response = await axios.get<ComicsData>(
      `${process.env.MARVEL_PUBLIC_URl}/comics?ts=${ts}&apikey=${apiKeyPublic}&hash=${hash}`
    );
    return response.data;
  } catch (error) {
    console.error("Error fetching characters: ", error);
...
export function getErrorCreateUser(code: string): string {
  const errorsAuthCreate: Record<string, string> = {
    ...
    "auth/invalid-password":
      "La contraseña debe ser una cadena con al menos 6 caracteres.",
  };
